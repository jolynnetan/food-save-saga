...
import { ShoppingCart, TrendingDown, AlertTriangle, Check, Minus, Plus, Lightbulb } from "lucide-react";
            ...
            <div key={insight.item} className="bg-warning/5 border border-warning/15 rounded-xl p-3 space-y-2">
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex gap-2">
                  <button
                    onClick={() => acceptSuggestion(insight.item)}
                    className="text-xs font-medium px-3 py-1.5 rounded-lg bg-primary text-primary-foreground transition-all active:scale-[0.95]"
                  >
                    Yes, reduce it
                  </button>
                  <button
                    onClick={() => acceptSuggestion(insight.item)}
                    className="text-xs font-medium px-3 py-1.5 rounded-lg bg-muted text-muted-foreground transition-all active:scale-[0.95]"
                  >
                    Keep as is
                  </button>
                </div>
            ...
            >
              <div
                className={`w-5 h-5 rounded-full border-2 flex items-center justify-center shrink-0 transition-colors ${
                  item.checked ? "bg-success border-success" : "border-muted-foreground/30"
                }`}
              >
